...
namespace Dungeon_Generator
{
	...
	{
		[SerializeField, InLineEditor] DungeonLayoutScriptable dungeonLayout;
		...
		class Dungeon
		{
			...
			public void Generate()
			{
				...
				for (int x = 0; x < map_size_x - 1; x++)
				{
					for (int y = 0; y < map_size_y - 1; y++)
					{
						if (walls.Contains(map[x, y + 1].type) && walls.Contains(map[x + 1, y].type) &&
						    roomsandfloors.Contains(map[x + 1, y + 1].type))
						{
							//north
							map[x, y].type = 4;
						...
						if (y > 0)
						{
							if (walls.Contains(map[x + 1, y].type) && walls.Contains(map[x, y - 1].type) &&
							    roomsandfloors.Contains(map[x + 1, y - 1].type))
							{
								//north
								map[x, y].type = 5;
		...
		[Button("Clear Old Dungeon", "Clears the old dungeon")]
		void ClearOldDungeon()
		{
			ClearOldDungeon(!Application.isPlaying);
		}
		...
		[Button("Generate", "Generates the Dungeon")]
		public void Generate()
		{
			...
			for (var y = 0; y < Dungeon.map_size_y; y++)
			{
				for (var x = 0; x < Dungeon.map_size_x; x++)
				{
					...
					if (tile == 1)
					{
						GameObject floorPrefabToUse = dungeonLayout.floorPrefab;
						...
						created_tile =
							GameObject.Instantiate(floorPrefabToUse, tile_location, Quaternion.identity) as GameObject;
					}

					if (Dungeon.walls.Contains(tile))
					{
						GameObject wallPrefabToUse = dungeonLayout.wallPrefab;
						Room room = Dungeon.map[x, y].room;
						if (room != null)
						{
							foreach (CustomRoom customroom in customRooms)
						...
						created_tile =
							GameObject.Instantiate(wallPrefabToUse, tile_location, Quaternion.identity) as GameObject;
						if (!makeIt3d)
						{
					...
					if (tile == 3)
					{
						created_tile = GameObject.Instantiate( /*floorPrefab*/dungeonLayout.corridorFloorPrefab, tile_location,
							Quaternion.identity) as GameObject;
					}

					if (Dungeon.corners.Contains(tile))
					{
						GameObject cornerPrefabToUse = dungeonLayout.cornerPrefab;
						Room room = Dungeon.map[x, y].room;
						if (room != null)
						{
							foreach (CustomRoom customroom in customRooms)
						...
						if (dungeonLayout.cornerPrefab)
						{
							created_tile =
								GameObject.Instantiate(cornerPrefabToUse, tile_location, Quaternion.identity) as
									GameObject;
							if (cornerRotation)
							{
								if (!makeIt3d)
						...
						{
							created_tile =
								GameObject.Instantiate(dungeonLayout.wallPrefab, tile_location, Quaternion.identity) as GameObject;
						}
					}
			...
			if (!makeIt3d)
			{
				end_point = GameObject.Instantiate(dungeonLayout.exitPrefab,
					new Vector3(Dungeon.goalRoom.x * tileScaling, Dungeon.goalRoom.y * tileScaling, 0),
					Quaternion.identity) as GameObject;
				start_point = GameObject.Instantiate(dungeonLayout.startPrefab,
					new Vector3(Dungeon.startRoom.x * tileScaling, Dungeon.startRoom.y * tileScaling, 0),
					Quaternion.identity) as GameObject;
			...
			{
				end_point = GameObject.Instantiate(dungeonLayout.exitPrefab,
					new Vector3(Dungeon.goalRoom.x * tileScaling, 0, Dungeon.goalRoom.y * tileScaling),
					Quaternion.identity) as GameObject;
				start_point = GameObject.Instantiate(dungeonLayout.startPrefab,
					new Vector3(Dungeon.startRoom.x * tileScaling, 0, Dungeon.startRoom.y * tileScaling),
					Quaternion.identity) as GameObject;
			}
			...
			for (int x = 0; x < Dungeon.map_size_x; x++)
			{
				for (int y = 0; y < Dungeon.map_size_y; y++)
				{
					if (Dungeon.map[x, y].type == 1 &&
					    ((Dungeon.startRoom != Dungeon.map[x, y].room && Dungeon.goalRoom != Dungeon.map[x, y].room) ||
					     maximumRoomCount <= 3))
					{
						...
						if (Dungeon.walls.Contains(Dungeon.map[x + 1, y].type) ||
						    Dungeon.walls.Contains(Dungeon.map[x - 1, y].type) ||
						    Dungeon.walls.Contains(Dungeon.map[x, y + 1].type) ||
						    Dungeon.walls.Contains(Dungeon.map[x, y - 1].type))
						{
							location.byWall = true;
						}

						if (Dungeon.map[x + 1, y].type == 3 || Dungeon.map[x - 1, y].type == 3 ||
						    Dungeon.map[x, y + 1].type == 3 || Dungeon.map[x, y - 1].type == 3)
						{
							location.byCorridor = true;
						}

						if (Dungeon.map[x + 1, y + 1].type == 3 || Dungeon.map[x - 1, y - 1].type == 3 ||
						    Dungeon.map[x - 1, y + 1].type == 3 || Dungeon.map[x + 1, y - 1].type == 3)
						{
							location.byCorridor = true;
						}
			...
			if (dungeonLayout.doorPrefab)
			{
				for (int i = 0; i < spawnedObjectLocations.Count; i++)
				{
					if (spawnedObjectLocations[i].asDoor > 0)
					{
						...
						GameObject doorPrefabToUse = dungeonLayout.doorPrefab;
						...
						if (!makeIt3d)
						{
							newObject = GameObject.Instantiate(doorPrefabToUse,
								new Vector3(spawnLocation.x * tileScaling, spawnLocation.y * tileScaling, 0),
								Quaternion.identity) as GameObject;
						}
						else
						{
							newObject = GameObject.Instantiate(doorPrefabToUse,
								new Vector3(spawnLocation.x * tileScaling, 0, spawnLocation.y * tileScaling),
								Quaternion.identity) as GameObject;
						}

						if (!makeIt3d)
						{
							newObject.transform.Rotate(Vector3.forward *
							                           (-90 * (spawnedObjectLocations[i].asDoor - 1)));
						}
						else
						{
				...
				while (objectCountToSpawn > 0)
				{
					...
					for (int i = 0; i < spawnedObjectLocations.Count; i++)
					{
						...
						if (createHere)
						{
							...
							if (!makeIt3d)
							{
								newObject = GameObject.Instantiate(objectToSpawn.gameObject,
									new Vector3(spawnLocation.x * tileScaling, spawnLocation.y * tileScaling, 0),
									Quaternion.identity) as GameObject;
							}
							else
		...
		public void Generate(DungeonLayoutScriptable dungeonLayout)
		{
		...
		{
			return new Vector3(Dungeon.startRoom.x, 0, Dungeon.startRoom.y);
		}
