...
{
  public sealed class GraphIterator
  {
    ...
    private readonly StateGraph tree;
    ...
    {
      get { return traversal.Count == 0 ? StateNode.kInvalidOrder : traversal.Peek(); }
    }
    ...
    /// </summary>
    public State? LastChildExitStatus { get; private set; }

    public State LastExecutedStatus { get; private set; }
    ...
    public GraphIterator(StateGraph tree, int levelOffset)
    {
      this.tree = tree;

      // Since tree heights starts from zero, the stack needs to have treeHeight + 1 slots.
      int maxTraversalLength = this.tree.nodes.Count + 1;
    ...
    public void Update()
    {
      ...
      StateNode node = tree.nodes[index] as StateNode;
      State s = node.Execute();
    ...
    private void CallOnEnterOnQueuedNodes()
    {
      ...
      while (requestedTraversals.Count != 0)
      {
        ...
        StateNode node = tree.nodes[i] as StateNode;
        node.OnEnter();
        OnChildEnter(node);
      }
    }

    private void OnChildEnter(StateNode node)
    {
      if (node.Parent)
      {
    ...
    private void OnChildExit(StateNode node, State s)
    {
      if (node.Parent)
      {
    ...
    public void Traverse(StateNode next)
    {
    ...
    public void AbortRunningChildBranch(StateNode parent, int abortBranchIndex)
    {
    ...
    internal void Interrupt(StateNode subtree)
    {
      // Keep interrupting up to the parent of subtree. 
      // The parent is not interrupted; subtree node is interrupted.
      if (subtree)
      {
        int parentIndex = subtree.Parent ? subtree.Parent.preOrderIndex : StateNode.kInvalidOrder;
